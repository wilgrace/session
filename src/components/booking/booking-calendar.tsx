...
function isEventFull(event: CalendarEvent): boolean {
  ...
  const effectiveCapacity = (instance as any).effectiveCapacity ?? event.resource.capacity ?? 10
    ...
    <div className="session-event-content">
      <div className="session-meta flex justify-between items-center">
        <span className="flex items-center gap-1">
          ...
          {event.isBooked ? 'Booked' : isFull ? 'Full • Join waiting list' : `${availableSpots} spaces` }
        </span>
        <span className="flex items-center gap-1">
